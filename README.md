...
**主要分頁（深色鋼鐵人 HUD）**
- **自選股**：伺服器持久化清單與自訂分類（例如航太股、記憶體股）、今日即時價與漲跌、開高低量、技術趨勢、健診、籌碼與停損停利摘要；卡片表面另有流動性／波動／追高／籌碼／基本面五面向風險快篩（缺資料不當成安全）
...
自選股與分類都儲存在執行這個服務的伺服器。若以 Synology 為唯一正式來源，Mac、手機都直接開啟同一個 Synology 網址，就會共用同一份自選股與分類；若另外在 Mac 啟動本機版，則仍會維持 Mac 自己獨立的一份資料。
